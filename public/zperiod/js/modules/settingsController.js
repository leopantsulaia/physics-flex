...
const ANIMATION_PAUSED_KEY = "leophysics_anim_paused";
const ANIMATION_SPEED_KEY = "leophysics_anim_speed";
...
function initSettingsSuggestionForm() {
  ...
  const sendSuggestion = async () => {
    ...
    await submitSuggestion(text, { source: "LeoPhysics Settings" });
...
function initAnimationControls() {
  ...
  if (speedSlider) {
    speedSlider.value = window._leophysicsAnimSpeed;
    if (speedLabel) {
      speedLabel.textContent = `${window._leophysicsAnimSpeed.toFixed(1)}×`;
    }
  }

  if (playToggle) {
    updatePlayToggleIcon(playToggle, window._leophysicsAnimPaused);
  }

  applyAnimationPauseState(window._leophysicsAnimPaused);

  if (playToggle) {
    playToggle.addEventListener("click", () => {
      window._leophysicsAnimPaused = !window._leophysicsAnimPaused;
      localStorage.setItem(ANIMATION_PAUSED_KEY, window._leophysicsAnimPaused);
      updatePlayToggleIcon(playToggle, window._leophysicsAnimPaused);
      applyAnimationPauseState(window._leophysicsAnimPaused);
    });
  }
  ...
  if (speedSlider) {
    speedSlider.addEventListener("input", () => {
      ...
      window._leophysicsAnimSpeed = value;
...
function initPreferencesCard() {
  ...
  if (reduceToggle) {
    const isReduced = localStorage.getItem("leophysics_reduce_motion") === "true";
    reduceToggle.checked = isReduced;
    
    reduceToggle.addEventListener("change", (e) => {
      localStorage.setItem("leophysics_reduce_motion", e.target.checked);
