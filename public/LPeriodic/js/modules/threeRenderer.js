...
export function ensureThreeLibLoaded() {
  ...
  threeLoadPromise = new Promise((resolve, reject) => {
    const existing = document.querySelector('script[data-leophysics-three="1"]');
    if (existing) {
    ...
    script.async = true;
    script.dataset.leophysicsThree = "1";
...
export function updateAtomStructure(element) {
  ...
  if (atomicNumber > 1) {
    ...
    if (atomicNumber === 11 && window._leophysicsNaMode === "cloud") lightIntensity = 5.0;
...
export function animateAtom() {
  ...
  const isPaused = window._leophysicsAnimPaused || false;
  const speedMul = (typeof window._leophysicsAnimSpeed === "number") ? window._leophysicsAnimSpeed : 0.6;
  ...
  const isEdu = isNa && window._leophysicsNaMode === "educational";
