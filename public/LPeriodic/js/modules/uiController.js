...
// v2 dataset stub — file removed; branches gated by leophysicsVersion === 'new' are inert.
...
let savedUnits = null;
try {
  const stored = localStorage.getItem('leophysics_units');
...
function saveUnits() {
  try {
    localStorage.setItem('leophysics_units', JSON.stringify(l3UnitState));
  } catch (e) {
...
function populateSimplifiedView(element) {
  ...
  if (greenCard) {
    ...
    if (window.leophysicsVersion === 'new' && v2Data) {
  ...
  if (yellowCard) {
    ...
    if (window.leophysicsVersion === 'new' && v2Data && v2Data.level2_atomic.mass.standard) {
    ...
    if (window.leophysicsVersion === 'new' && v2Data && v2Data.level3_properties?.electronic?.configuration) {
  ...
  if (redCard) {
    ...
    if (window.leophysicsVersion === 'new' && v2Data && v2Data.level4_history_stse.history.discoveryYear) {
...
export function showModal(element) {
  ...
  initElementTutorial();
  document.title = `LeoPhysics - ${localizeElementName(element)}`;
