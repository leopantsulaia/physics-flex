...
/**
 * Enhanced Onboarding Flow for LeoPhysics 2.0
 */
export function initOnboardingFlow() {
  const overlay = document.createElement("div");
  overlay.id = "leophysics-onboarding-overlay";
  overlay.innerHTML = `
    <style>
      #leophysics-onboarding-overlay {
        position: fixed;
        inset: 0;
        background: #ffffff;
        z-index: 1000000;
    ...
    <div id="animation-stage" style="display:flex; flex-direction:column; align-items:center;">
        <div class="logo-container" id="onboarding-logo">
          ...
          <span class="logo-text">LeoPhysics</span>
        </div>
  ...
  function startDanmaku() {
    ...
    if (allPhrases.length === 0) {
        allPhrases.push("LeoPhysics", "Chemistry", "Atom", "Molecule");
    }
  ...
  langScreen.addEventListener("click", (e) => {
    ...
    localStorage.setItem("leophysics_welcomed_v2", "true");
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        localStorage.setItem("leophysics_lang", lang);
