...
import { AlgorithmsView } from "./AlgorithmsView"; // Import the new file

// --- EXPANDED ISOTOPE DATABASE ---
const ISOTOPES = {
  ...
  "Lu-177": { name: "Lutetium-177", halfLife: 159.52 }, // 6.647 days * 24
...
export const DecayClock = () => {
  // ... [Keep all existing state variables exactly as they were] ...
  ...
  const [elutionActivity, setElutionActivity] = useState(1000);
  const [calcMode, setCalcMode] = useState("FIND_TIME");
  const [targetDoseInput, setTargetDoseInput] = useState(500);
  const [targetDateInput, setTargetDateInput] = useState(new Date().toISOString().slice(0, 10));
  const [targetTimeInput, setTargetTimeInput] = useState(new Date(Date.now() + 3600000).toTimeString().slice(0, 5));
  const [currentActivity, setCurrentActivity] = useState(1000);
  const [calculationResult, setCalculationResult] = useState({ mainValue: "", subValue: "", isOverdue: false, label: "" });
  
  // NEW STATE FOR ALGORITHMS VIEW
  const [showAlgorithms, setShowAlgorithms] = useState(false);

  // ... [Keep setElutionToNow helper] ...
  ...
  // ... [Keep useEffect and calculatePhysics exactly as they were] ...
  useEffect(() => {
    const interval = setInterval(() => calculatePhysics(), 1000);
    calculatePhysics();
    return () => clearInterval(interval);
  }, [isotope, elutionDate, elutionTime, elutionActivity, calcMode, targetDoseInput, targetDateInput, targetTimeInput]);
  ...
  const calculatePhysics = () => {
    ...
    if (calcMode === "FIND_TIME") {
      if (targetDoseInput >= elutionActivity) {
        setCalculationResult({ label: "IMPOSSIBLE", mainValue: "Target > Source", subValue: "Cannot decay up", isOverdue: true });
      } else {
        ...
        setCalculationResult({ label: "EXPECTED AT TIME", mainValue: targetDateObj.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}), subValue: targetDateObj.toLocaleDateString(), isOverdue: isPast });
      }
      ...
      if (hoursFromSource < 0) {
        setCalculationResult({ label: "ERROR", mainValue: "Time < Elution", subValue: "Check dates", isOverdue: true });
      } else {
        const futureDose = elutionActivity * Math.exp(-lambda * hoursFromSource);
        const isPast = targetTimestamp < now;
        setCalculationResult({ label: "EXPECTED DOSE", mainValue: `${futureDose.toFixed(2)} MBq`, subValue: isPast ? "(Historical Value)" : "(Future Value)", isOverdue: isPast });
      }
    }
  };
  ...
  return (
    <div className={styles.container}>
      {/* RENDER ALGORITHMS OVERLAY IF ACTIVE */}
      {showAlgorithms && <AlgorithmsView onClose={() => setShowAlgorithms(false)} />}

      <h1 className={styles.title}>☢️ ATOMIC DECAY CLOCK</h1>

      <div className={styles.gridContainer}>
        {/* ... [Keep Controls and Display Columns exactly as they were] ... */}
        <div className={styles.controls}>
          <div className={styles.sectionTitle}>SOURCE SETUP</div>
          <div className={styles.controlItem}>
            <label className={styles.label}>ISOTOPE</label>
            <select value={isotope} onChange={(e) => setIsotope(e.target.value)} className={styles.select}>
              {Object.keys(ISOTOPES).map((iso) => <option key={iso} value={iso}>{ISOTOPES[iso].name}</option>)}
            </select>
          </div>
          <div className={styles.row}>
            <div className={styles.controlItem}>
              ...
              <div className={styles.inputGroup}>
                <input type="time" value={elutionTime} onChange={(e) => setElutionTime(e.target.value)} className={styles.input} />
                <button onClick={setElutionToNow} className={styles.nowBtn}><b>NOW</b></button>
              </div>
            </div>
            <div className={styles.controlItem}>
              <label className={styles.label}>ACTIVITY (MBq)</label>
              <input type="number" value={elutionActivity} onChange={(e) => setElutionActivity(Number(e.target.value))} className={styles.input} />
            </div>
          </div>
          <div className={styles.divider}></div>
          <div className={styles.sectionTitle}>CALCULATOR</div>
          <div className={styles.toggleContainer}>
            <button className={`${styles.toggleBtn} ${calcMode === "FIND_TIME" ? styles.active : ""}`} onClick={() => setCalcMode("FIND_TIME")}>FIND TIME</button>
            <button className={`${styles.toggleBtn} ${calcMode === "FIND_DOSE" ? styles.active : ""}`} onClick={() => setCalcMode("FIND_DOSE")}>FIND DOSE</button>
          </div>
          {calcMode === "FIND_TIME" ? (
            <div className={styles.controlItem}>
              <label className={styles.label}>TARGET DOSE (MBq)</label>
              <input type="number" value={targetDoseInput} onChange={(e) => setTargetDoseInput(Number(e.target.value))} className={`${styles.input} ${styles.highlightInput}`} />
            </div>
          ) : (
            <div className={styles.row}>
              <div className={styles.controlItem}>
                <label className={styles.label}>TARGET DATE</label>
                <input type="date" value={targetDateInput} onChange={(e) => setTargetDateInput(e.target.value)} className={`${styles.input} ${styles.highlightInput}`} />
              </div>
              <div className={styles.controlItem}>
                <label className={styles.label}>TARGET TIME</label>
                <input type="time" value={targetTimeInput} onChange={(e) => setTargetTimeInput(e.target.value)} className={`${styles.input} ${styles.highlightInput}`} />
              </div>
            </div>
          )}
        </div>
        ...
        <div className={styles.displayColumn}>
          <div className={styles.liveBox}>
            ...
            <div className={styles.activityValue}>{currentActivity.toFixed(2)} <span className={styles.unit}>MBq</span></div>
            <div className={styles.progressBarContainer}>
              <div className={styles.progressBar} style={{ width: `${Math.min((currentActivity / elutionActivity) * 100, 100)}%`, backgroundColor: '#0f0' }}></div>
            </div>
          </div>
          <div className={`${styles.resultBox} ${calculationResult.isOverdue ? styles.overdue : ''}`}>
            ...
            {calculationResult.isOverdue && <div className={styles.warningTag}>⚠️ PASSED / HISTORICAL</div>}
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: '40px', textAlign: 'center', borderTop: '1px solid #333', paddingTop: '20px' }}>
        <button 
          ...
          style={{ background: 'transparent', border: 'none', color: '#333', cursor: 'pointer', fontSize: '12px', fontFamily: 'Courier New' }}
        >
