...
const PhysicsBox = ({ formula, notes }) => (
  <div style={{ margin: '20px 0', padding: '22px', borderLeft: `3px solid ${T.green}`, background: T.panel }}>
    <div style={{ fontSize: '11px', color: T.dim, marginBottom: '10px', letterSpacing: '1px' }}>
      THE PHYSICS — FIRST PRINCIPLES
    </div>
    <p style={{ fontSize: '20px', letterSpacing: '1px', margin: '0 0 12px', color: '#fff' }}>
      {formula}
    </p>
    {notes && (
...
const CodeBox = ({ title, code }) => (
  <div style={{ margin: '20px 0', padding: '22px', background: '#141414', borderRadius: '6px', border: `1px solid ${T.border}` }}>
    <div style={{ fontSize: '11px', color: '#aaa', marginBottom: '14px', letterSpacing: '1px' }}>
      {title || 'JAVASCRIPT IMPLEMENTATION'}
    </div>
    <pre style={{ whiteSpace: 'pre-wrap', color: '#e8e8e8', fontSize: '13px', margin: 0, lineHeight: 1.7, fontFamily: T.mono }}>
      {code}
    </pre>
  </div>
);
...
const REFERENCES = [
  ...
  // ICRP
  { id: 6, cat: 'ICRP', text: 'ICRP Publication 139 — Occupational Radiological Protection in Interventional Procedures. Ann. ICRP 47(2), 2018. (Lens limit: 20 mSv/yr; thyroid shield guidance.)' },
  ...
  // AAPM
  { id: 13, cat: 'AAPM', text: 'AAPM Task Group 191 (TG-191) — Clinical Use of Luminescent Dosimeters. Med. Phys. 2021. (Lead-equivalent specifications for leaded glasses: 0.75 mm Pb-eq.)' },
  { id: 14, cat: 'AAPM', text: 'AAPM Task Group 108 — PET and PET/CT Shielding Requirements. Med. Phys. 33(1), 2006; 2020 guidance update. (F-18, Ga-68 shielding data used in this tool.)' },
  ...
  { id: 21, cat: 'J', text: 'Stabin MG — Fundamentals of Nuclear Medicine Dosimetry. Springer, 2020. (Decay constant and activity equations, Ch. 2.)' },
  ...
  { id: 24, cat: 'J', text: 'Berger MJ, Hubbell JH et al. — XCOM: Photon Cross Sections Database. NIST Standard Reference Database 8. NIST, updated 2022. (Attenuation coefficients for Pb, W, concrete.)' },
...
const catColor = { IAEA: T.blue, ICRP: T.gold, NCRP: T.green, AAPM: '#c39bd3', J: T.dim };
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: '26px', letterSpacing: '2px' }}>NAKED ALGORITHMS</h1>
            <div style={{ fontSize: '11px', opacity: 0.6, marginTop: '4px' }}>
              VERIFICATION PROTOCOL v2.0 — NucMed Shielding Optimizer
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <Section number="1.0" title="RADIOACTIVE DECAY ENGINE">
          <PhysicsBox
            formula={<>A(t) = A₀ · e<sup>−λt</sup></>}
            notes={<>
              <strong>Derivation:</strong> The rate of decay (−dN/dt) is proportional to the number of
              undecayed nuclei N: <em>dN/dt = −λN</em>. Separating variables and integrating gives
              N(t) = N₀·e<sup>−λt</sup>. Since activity A = λN, we get A(t) = A₀·e<sup>−λt</sup>.<br /><br />
              <strong>Decay constant λ (lambda):</strong> λ = ln(2) / T½ = <em>Math.LN2 / T½</em><br />
              Using <code>Math.LN2</code> (= 0.6931471805599453…) is more precise than the hardcoded
              approximation 0.693 — a difference of 0.002% per half-life, which compounds over many
              half-lives.<br /><br />
              <strong>Variables:</strong><br />
              A₀ = initial activity at t=0 (MBq)<br />
              λ = decay constant (hr⁻¹)<br />
              t = elapsed time (hours)<br />
              T½ = half-life (hours)
            </>}
          />
          <CodeBox code={
            `// ─── STEP 1: Compute decay constant (lambda) ───────────────────────────
...
const safeActivity = Math.max(currentActivity, 0);`
          ...
          <CodeBox title="INVERSE CALCULATION — Find time when activity reaches target" code={
            `// If you know A₀, A_target and λ, solve for t:
//   A_target = A₀ · e^(−λt)
//   ln(A_target / A₀) = −λt
//   t = −ln(A_target / A₀) / λ
...
const targetTimestamp = startTime + hoursToTarget * 3600 * 1000; // back to ms`
        ...
        <Section number="2.0" title="DISTANCE ATTENUATION (INVERSE SQUARE LAW)">
          <PhysicsBox
            formula={<>H&#775; = Γ · A / d²</>}
            notes={<>
              <strong>Derivation:</strong> A point source radiates isotropically across a spherical
              shell of area 4πd². As d increases, intensity spreads over a larger area, so dose rate
              falls as 1/d². Combining with the source-specific gamma constant Γ:<br /><br />
              H&#775; [mR/hr] = Γ [R·cm²/mCi·hr] × A [mCi] / d² [m²]<br /><br />
              The factor of 10 in the code converts the unit mismatch (Γ is in cm² but d is in m²;
              1 m² = 10,000 cm² → coefficient = 10,000/1000 = 10).<br /><br />
              <strong>Gamma constants (Γ) used in this tool are sourced from:</strong><br />
              Shultis &amp; Faw — <em>Radiation Shielding</em>, 2000; NCRP Report 151 (2005/2021 reprint).
            </>}
          />
          <CodeBox code={
            `// ─── STEP 1: Normalize activity to mCi ─────────────────────────────────
...
const unshieldedDoseRate =
  (gammaConstant * 10 * activityInMci) / (distanceM ** 2);`
        ...
        <Section number="3.0" title="SHIELDING ATTENUATION (HVL METHOD)">
          <PhysicsBox
            formula={<>I = I₀ · (½)<sup>n</sup> = I₀ · e<sup>−μx</sup></>}
            notes={<>
              <strong>Two equivalent forms:</strong><br />
              1. <strong>HVL form:</strong> n = x / HVL gives the number of half-value layers.
              Each HVL halves the intensity, so I = I₀·(1/2)^n.<br /><br />
              2. <strong>Exponential form:</strong> I = I₀·e<sup>−μx</sup> where μ is the
              linear attenuation coefficient (cm⁻¹). HVL = ln(2)/μ.<br /><br />
              This tool uses the HVL form because HVL values are more commonly tabulated
              in nuclear medicine references and are easier to verify clinically.<br /><br />
              <strong>Note:</strong> The simple HVL formula is a <em>narrow-beam</em> approximation.
              In reality, scattered photons survive into the beam (buildup factor B &gt; 1).
              For conservative room-shielding estimates, this narrow-beam approach is appropriate
              as it slightly over-estimates required thickness (safer side).
            </>}
          />
          <CodeBox code={
            `// ─── STEP 1: Required attenuation factor ───────────────────────────────
...
const attenuatedDoseRate = (thicknessMm, hvlMm) => {
  ...
  return unshieldedDoseRate * Math.pow(0.5, n); // dose after PPE
        ...
        <Section number="4.0" title="PERSONAL PROTECTIVE EQUIPMENT (PPE)">
          <div style={{ margin: '20px 0', padding: '22px', borderLeft: `3px solid ${T.red}`, background: `${T.red}08` }}>
            <div style={{ fontSize: '11px', color: T.red, marginBottom: '12px', letterSpacing: '1px' }}>
              CLINICAL RATIONALE & HOT-LAB REQUIREMENTS
            </div>
            <p style={{ color: T.text, fontSize: '13.5px', lineHeight: 1.8, margin: 0 }}>
              In nuclear medicine hot-lab settings — where staff handle multi-gigabecquerel activities of
              I-131, Tc-99m generators, PET radiopharmaceuticals (F-18, Ga-68), and therapeutic Lu-177 —
              personnel protective equipment provides meaningful dose reduction even though it does <em>not</em> eliminate
              exposure. The principle of <strong style={{ color: T.green }}>ALARA</strong> (As Low As Reasonably Achievable)
              mandates their consistent use.
            </p>
          </div>

          {/* PPE TABLE */}
          <div style={{ margin: '20px 0', padding: '20px', background: '#0e0e0e', border: `1px solid ${T.border}`, borderRadius: '6px' }}>
            <div style={{ fontSize: '11px', color: '#aaa', marginBottom: '14px', letterSpacing: '1px' }}>
              PPE SPECIFICATIONS — LEAD-EQUIVALENT THICKNESS & ATTENUATION
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12.5px', fontFamily: T.mono }}>
              <thead>
                <tr style={{ color: T.dim, borderBottom: `1px solid ${T.border}` }}>
                  <th style={{ textAlign: 'left', padding: '6px 8px' }}>EQUIPMENT</th>
                  <th style={{ textAlign: 'center', padding: '6px 8px' }}>Pb-eq</th>
                  <th style={{ textAlign: 'center', padding: '6px 8px' }}>Tc-99m attenuation</th>
                  <th style={{ textAlign: 'left', padding: '6px 8px' }}>SOURCE</th>
                </tr>
              </thead>
              <tbody style={{ color: T.text }}>
                {[
                  ['Lead Apron (standard)', '0.35 mm', '~90%', 'NCRP 168 (2021)'],
                  ['Lead Apron (hot-lab)', '0.50 mm', '~94%', 'IAEA Safety Series No. 40'],
                  ['Thyroid Shield', '0.35 mm', '~90%', 'ICRP 139 (2018)'],
                  ['Leaded Glasses (lens)', '0.75 mm Pb-eq', '~97%', 'AAPM TG-191 (2021)'],
                  ['Lead-Glass Bench Shield', '2.00 mm Pb-eq', '~99.9%', 'Typical spec (industry)'],
                ].map(([eq, spec, att, src], i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${T.border}` }}>
                    <td style={{ padding: '8px 8px', color: '#fff' }}>{eq}</td>
                    <td style={{ padding: '8px 8px', textAlign: 'center', color: T.gold }}>{spec}</td>
                    <td style={{ padding: '8px 8px', textAlign: 'center', color: T.green }}>{att}</td>
                    <td style={{ padding: '8px 8px', color: T.dim, fontSize: '11px' }}>{src}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div style={{ marginTop: '14px', fontSize: '11.5px', color: T.dim, lineHeight: 1.7 }}>
              * Attenuation for Tc-99m (140 keV primary + scatter). Values decrease for higher-energy isotopes
              (e.g., I-131 at 364 keV, F-18 at 511 keV annihilation photons). Consult the shielding calculator
              results for isotope-specific PPE dose estimates.
            </div>
          </div>

          <div style={{ margin: '20px 0', padding: '22px', background: `${T.red}10`, border: `1px solid ${T.red}44`, borderRadius: '6px' }}>
            <div style={{ color: T.red, fontWeight: 'bold', marginBottom: '12px', fontSize: '13px' }}>
              🔴 THYROID SHIELD — WHY MANDATORY IN HOT-LAB (EVEN AT 0.35 mm)
            </div>
            <ul style={{ color: T.text, fontSize: '13px', lineHeight: 1.9, margin: 0, paddingLeft: '20px' }}>
              <li>The <strong>thyroid gland</strong> has a radiation weighting factor w<sub>T</sub> = 0.04 (ICRP 103) — small, but its dose limit is 500 mSv/year (occupational extremity class).</li>
              <li>In I-131 hot-labs, the primary exposure pathways include <strong>direct gamma</strong> (364 keV), but also <strong>inhaled/aerosolized radioiodine</strong> — the thyroid collar provides a supplementary layer even if the primary risk is internal.</li>
              <li>Even a thin 0.35 mm Pb collar reduces scattered Tc-99m (140 keV) dose to the thyroid by <strong>~60–90%</strong> depending on geometry.</li>
              <li>ICRP Publication 139 (2018) explicitly recommends thyroid shields for staff performing &gt;400 MBq I-131 administrations regularly.</li>
              <li><strong>Dose asymmetry:</strong> The neck/thyroid region is not covered by a standard lead apron — it is <em>entirely unprotected</em> without a dedicated collar.</li>
            </ul>
          </div>

          <CodeBox title="PPE IMPLEMENTATION IN THIS TOOL" code={
            `// Each PPE item has a DISTINCT, clinically-validated lead-equivalent
...
Object.entries(PPE_SPECS).forEach(([item, thickMm]) => {
  ...
  console.log(\`\${item}: \${reducedDose.toFixed(3)} mR/hr\`);
        ...
        <Section number="5.0" title="REFERENCES (2020 – 2026)">
          <div style={{ margin: '20px 0', padding: '4px 0', fontSize: '12px', color: T.dim }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {REFERENCES.map(ref => (
              <div key={ref.id} style={{
                ...
                <div style={{ color: T.dimmer, fontSize: '11px', paddingTop: '1px' }}>
                  [{ref.id}]
                </div>
                <div>
        ...
        <div style={{
          ...
          <p style={{ margin: '0 0 4px', fontSize: '13px' }}>ENGINEERED BY LEOPANTSULAIA</p>
