...
import { ISOTOPE_DATA, MBQ_TO_MCI } from "../constants/isotopeData";
...
export const useShieldingCalculation = (
  ...
  useEffect(() => {
    calculateShielding();
  }, [isotope, unit, inputValue, distance, targetDose]);
  ...
  const calculateShielding = () => {
    ...
    const unshieldedDoseRate =
      (data.gammaConstant * 10 * activityInMci) / (distance * distance);
